...
exports.create = async (req, res) => {
	try {
		let { email, password, confirm_password, restaurant } = req.body; // Getting required fields from body
		...
		if (existingManager) {
			...
			return res.status(409).json({ success: false, message: "Manager already exists." });
		...
		const manager = await Managers.create({ ...req.body, restaurant: result._id }); // Adding manager in db
...
exports.getById = async (req, res) => {
	try {
		...
		const manager = await Managers.findById(managerId); // Finding manager by id
		res.json({ success: true, manager }); // Success
	} catch (err) {
		// Error handling
		console.log("Error ----> ", err);
